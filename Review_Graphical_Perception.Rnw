...
\begin{subfigure}[b]{.4\linewidth}
  ...
  \caption{The Human Eye\label{fig:humaneyediagram}
...
\begin{subfigure}[b]{.59\linewidth}
  ...
  \caption{Retina Cell Layers\label{fig:retinacells}
...
\end{figure}
Figure  \ref{fig:gestaltlaws} shows examples of some of the gestalt laws, which when combined help to order our perceptual experience. We have discussed how we attend to visual stimuli and how they are recognized, but for the perceptual experience to be meaningful, previous experiences must be stored into memory in some coherent way. The next section discusses how visual scenes are stored into long-term memory.

\subsubsection{Visual Memory}
We have discussed how visual stimuli are perceived and how objects are recognized; we now must examine how visual stimuli are encoded into memory. Most researchers believe that visual perceptions are encoded in an analog fashion, so that the memory of an image is closely related to the perception of that same image \citep{cognition}. Other theories \citep{borkin2016beyond} suggest that visual perceptions are encoded semantically, that is, the description of a visual scene would be encoded, rather than a mental ``image" of that scene. Both theories are likely at least partially correct, but the analog encoding of visual images is more relevant to statistical graphics because the accuracy of the stored image has the potential to affect recall of the contents of that image (and thus what people remember about a particular graphic). The mental rotation task provides experimental evidence to support analog encoding. In this task, participants must determine whether or not a figure is a rotation of a target figure, as shown in Figure  \ref{fig:mentalRotation}. \citet{shepard1988mental} showed that reaction time was proportional to the angle of rotation of the stimuli, which suggests that participants mentally rotate the figure as they would rotate a three-dimensional figure in space.

\begin{figure}[htbp]\centering
\includegraphics[width=.5\linewidth]{RotationTask}
\caption[Rotation task]{Rotation task \protect\citep{shepard1988mental}. Are the two images the same?
%https://en.wikipedia.org/wiki/File:MR_TMR.jpg (Edited)
}\label{fig:mentalRotation}
\end{figure}
...
\caption[Colorblindness and rainbow color schemes]{Rainbow color scheme with simulations of each of the six common types of color deficiency. The original image, on the left, is from \protect\citet{light2004end}. The top row of pictures on the right show simulations of the map in individuals who are entirely missing cones due to gene deletion or non-function. The bottom row of pictures show simulations of the map as viewed by individuals who have altered cones due to gene mutations. }\label{fig:colorblindrainbow}
\end{figure}
...
The second of these illusions is the cafe wall illusion, shown in Figure  \ref{fig:cafewall}, named because this tile pattern is apparently common in cafes. The cafe wall illusion is in part due to the contrast between light and dark zones (as in the mach bands and hermann grid illusion), and the strength of the illusion is reduced when the black and white tiles are replaced with isoluminant colors. Even with isoluminant colors, though, the illusion still remains. \citet{westheimer2007irradiation} suggests that this portion of the illusion is because the position of a black-white border will be biased to appear closer to the black side of its physical location, an effect which is compounded in the cafe wall illusion to produce the appearance of tilted lines. This illusion, while not explicitly found in most statistical graphics, shows that even simple (and pleasant) configurations of geometric objects can wreak havoc in the brain under the right circumstances. In fact, the illusion is so simple that it is also known as the ``Kindergarten illusion"\citep{pierce1898illusion}, but its cause is sufficiently complex that it has not been fully explained by psychologists or neuroscientists.
...
library(dichromat)
colors <- colorschemes$Categorical.12[c(2, 9)]
...
Task-based models suggest that task-based graphical perception (using a graph to answer a specific question) involves several stages of information processing \citep{ratwani2008thinking}.
\begin{enumerate}
...
<<clustering, fig.width=3, fig.height=3, echo=F, include=F>>=
N <- 40
...
qplot(data=data, x=x, y=y, color=group, shape = group) +
...
\todo{Figure out how to cite an unpublished paper... or just cite the experimental app?}
...
<<irisdatagrammar, echo=FALSE, include=FALSE, fig.width=2.5, fig.height=2.5>>=
# Density plot
...
Another useful tool for testing statistical graphics is the concept of a lineup. Lineups combine the psychological notion of visual search tasks with the statistical concept of hypothesis testing: Participants are provided with a number of plots of the same form, one using real data and the rest generated using resampling methods or model-based data generating methods. If participants identify the target plot (the plot with real data), this is considered similar in nature to a significant hypothesis test at a given $\alpha$ level (generally, there are 20 plots, so $\alpha=0.05 = 1/20$) \todo{add citation}. Figure  \ref{fig:lineupexample} shows a sample lineup.
...
\todo[inline]{Add conclusion - explain the importance of understanding how graphs are perceived so that statistical ideas can be presented effectively.}
